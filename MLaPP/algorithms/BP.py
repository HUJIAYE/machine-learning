...
def BP(data, label, hidden_number, step, model = 'normal'):
    ...
    input_hidden_para = np.random.rand(hidden_number, input_number)
    hidden_output_para = np.random.rand(output_number, hidden_number)
    #训练网络：
    if model == 'normal':
        ...
        for i in range(data.shape[0]):
            for j in range(hidden_number):
                hidden_x = np.sum(data[i] * input_hidden_para[j].T) - hidden_threshold[i, j]
                alpha[i,j] = sigmoid(hidden_x)
            for m in range(output_number):
                output_x = np.sum(alpha[i] * hidden_output_para[m].T) - output_threshold[i, m]
                beta[i, m] = sigmoid(output_x)
            error[i] = 0.5 * np.sum((beta[i] - label)**2)
            #更新参数
            delta_hidden_treshold = 
...
if __name__ == '__main__':
    x_train, x_test, y_train, y_test = load_data('C:\\Users\\Jiaye\\Desktop\\Jupyter\\MLaPP\\dataset\\iris.data')
